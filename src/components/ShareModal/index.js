...
const ShareModal = ({ showDialog, closeDialog }) => {
  ...
  const onSelect = (items) => {
    if (items) selectUsers([...items.map((item) => item.user)])
    else selectUsers([])
  }
  ...
  return (
    ...
    >
      ...
      <div className="dialog-content">
        ...
        <Checkbox className="body-1 m-0 py-5" name="one" value="share_all">
          ...
          <div className="shared-list">
            {sharedUsers.map((user, index) => (
              ...
              >
                ...
                <p
                  className="position-absolute text-grey"
      ...
      <div className="dialog-footer">
        <a href="" className="d-flex text-primary">
          <img src={LinkIcon} className="mr-2" /> Copy link
        </a>
        <Button
          className="ml-auto"
          onClick={() => (selectedUsers.length > 0 ? onShare() : onDone())}
        >
